...
#session = Session()
#cust = session.query(Customer).filter_by(id=5).first()
#print(cust)
#cust1 = session.query(Customer, Customer.full_money).filter_by(id=5).first()
#print(cust1)
#print(acc.info)
#print(cust.full_money)
#print(Customer.full_money)
#some_money = session.query(Customer).filter(Customer.full_money=='547 000 $10 802 000 RUB').first()
#print(some_money)
#br = session.query(Branch).first()
#br.full_address = ('New York', 'Maddyson Garden')
#session.add(br)
#session.commit()
#session.refresh(br)
#print(br.full_address)
#usr1 = User(fullname='asdas', email='zxc@zxc.ru', hashed_password='hashzxc', is_customer=True)
#prod1 = session.query(Product).filter_by(product_name='credit').first()
#for customer in prod1.customers:
#print(prod1.customers)
#cust1 = Customer(fullname='cust custov', city='saint-p', address='nikolskaya', email='cust@yahoo.com')
#cust1.products = [prod1]
#session.add(cust1)
#session.commit()
